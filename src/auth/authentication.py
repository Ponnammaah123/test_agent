    ...
    try:
        ...
        for field in required_fields:
            if field not in payload:
                raise HTTPException(
                    status_code=401, detail=f"Missing required field: {field}"
                )
        ...
        if not payload.get("appId") or not payload.get("appName"):
            raise HTTPException(
                status_code=401, detail="Invalid appId or appName in token"
            )
...
def load_public_key_from_env(env_var_name: str) -> serialization.PublicFormat:
    ...
    if not key_data:
        raise ValueError(
            f"The environment variable '{env_var_name}' is not set or empty."
        )
